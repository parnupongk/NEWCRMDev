	...
	p._initMorris = function () {
		...
		if ($('#morris-donut-graph').length > 0) {
			Morris.Donut({
				...
				data: [
					{value: 70, label: 'Positive', formatted: '70%'},
					{value: 20, label: 'Neutral', formatted: '20%'},
					{value: 10, label: 'Negative', formatted: '10%'}
				],
		...
		// Morris Bar Report demo
		if ($('#morris-bar-graph-report').length > 0) {
			Morris.Bar({
				element: 'morris-bar-graph-report',
				data: [
					{x: 'ออมสิน',y: 45},
					{x: 'ออมทรัพย์', y: 30},
					{x: 'เสียเวลา', y: 10},
				...
				barColors: function (row, series, type) {
					if(row.label == "ออมสิน") return "#AD1D28";
					else if(row.label == "ออมทรัพย์") return "#DEBB27";
					else if(row.label == "เสียเวลา") return "#fec04c";
					else if(row.label == "โกง") return "#1AB244";
					else if(row.label == "ปิดบัญชี") return "#3AA244";
				}
				//barColors: $('#morris-bar-graph-report').data('colors').split(',')
			});
		}
		...
		if ($('#morris-bar-graph').length > 0) {
			Morris.Bar({
				...
				barColors: function (row, series, type) {
					if(row.label == "Facebook") return "#3a5795";
					else if(row.label == "Twitter") return "#55acee";
					else if(row.label == "Pantip") return "#3c3963";
				}
			});
		}

		// Morris Top 5 Report demo
		if ($('#morris-donut-graph-top5').length > 0) {
			Morris.Donut({
				element: 'morris-donut-graph-top5',
				data: [
					{value: 40, label: 'สุเทพ เมพขิง', formatted: '40%'},
					{value: 25, label: 'สมชาย จรดปลายเท้า', formatted: '25%'},
					{value: 15, label: 'นิภา เชิดชู', formatted: '15%'},
					{value: 10, label: 'รัชดาภรณ์ สิมาแก้ว', formatted: '10%'},
					{value: 10, label: 'วรรณภา เสริมศรี', formatted: '10%'}
				],
				colors: $('#morris-donut-graph-top5').data('colors').split(','),
				formatter: function (x, data) {
					return data.formatted;
				}
			});
		...
		if ($('#morris-bar-graph-top5').length > 0) {
			Morris.Bar({
				element: 'morris-bar-graph-top5',
				data: [
					{x: 'สุเทพ เมพขิง', a: 90},
					{x: 'สมชาย จรดปลายเท้า', a: 25},
					{x: 'นิภา เชิดชู', a: 15},
					{x: 'รัชดาภรณ์ สิมาแก้ว', a: 10},
					{x: 'วรรณภา เสริมศรี', a: 10}
				],
				xkey: 'x',
				ykeys: ['a'],
				xLabelMargin: 0,
				labels: ['จำนวนผู้ติดตาม'],
				barColors: function (row, series, type) {
					if(row.label == "สุเทพ เมพขิง") return "#AD1D28";
					else if(row.label == "สมชาย จรดปลายเท้า") return "#DEBB27";
					else if(row.label == "นิภา เชิดชู") return "#fec04c";
					else if(row.label == "รัชดาภรณ์ สิมาแก้ว") return "#1AB244";
					else if(row.label == "วรรณภา เสริมศรี") return "#3AA244";
				}
			});
		}

		if ($('#morris-donut-graph-responded').length > 0) {
			Morris.Donut({
				element: 'morris-donut-graph-responded',
				data: [
					{value: 20, label: 'Total', formatted: '20%'},
					{value: 80, label: 'Responded', formatted: '80%'}
				],
				colors: $('#morris-donut-graph-responded').data('colors').split(','),
				formatter: function (x, data) {
					return data.formatted;
				}
			});
