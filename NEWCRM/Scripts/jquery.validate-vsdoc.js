...
$.extend($.validator, {
	...
	prototype: {
		...
		startRequest: function(element) {
			if (!this.Initiate[element.name]) {
				this.InitiateRequest++;
				this.Initiate[element.name] = true;
			}
		},
		
		stopRequest: function(element, valid) {
			this.InitiateRequest--;
			// sometimes synchronization fails, make sure InitiateRequest is never < 0
			if (this.InitiateRequest < 0)
				this.InitiateRequest = 0;
			delete this.Initiate[element.name];
			if ( valid && this.InitiateRequest == 0 && this.formSubmitted && this.form() ) {
				$(this.currentForm).submit();
				this.formSubmitted = false;
			} else if (!valid && this.InitiateRequest == 0 && this.formSubmitted) {
				$(this.currentForm).triggerHandler("invalid-form", [this]);
				this.formSubmitted = false;
			}
		},
	...
	if ( $.ajaxPrefilter ) {
		...
		$.ajax = function(settings) {
			...
			if (mode == "abort") {
			    ...
			    return (InitiateRequests[port] = ajax.apply(this, arguments));
		    }
